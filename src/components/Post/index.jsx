...
const Post = ({className, post, classVariant = 1}) => {
  const { title, mainImage, additionalImage, description, slug, publishedAt } = post;
  ...
  return (
    <Link href={`/post/${encodeURIComponent(slug.current)}`} className={cl(className, styles.post)}>
      <a className={cl(styles.post, classVariant === 2 && styles.postLeft, classVariant === 3 && styles.postTwo)}>
