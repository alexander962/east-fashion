...
const preloadImages = posts => {
  posts.slice(0, 6).forEach(post => {
    const imageUrl = urlFor(post?.mainImage).width(1000).url(); // Используем CDN и уточняем размер изображения
    const link = document.createElement('link');
    link.rel = 'preload';
    link.as = 'image';
    link.href = imageUrl;
        ...
        <a className={styles.favouritesPost}>
          <div className={styles.favouritesImg}>
            <Image
              ...
              layout="fill"
              objectFit="cover"
              src={urlFor(post?.mainImage).width(1000).url()} // Используем CDN и уточняем размер изображения
              alt=""
            />
          </div>
          <div className={styles.favouritesInfo}>
